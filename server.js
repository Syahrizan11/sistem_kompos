...
// ── TUKAR KATA LALUAN ─────────────────────────────────────────────
app.post('/api/admin/tukar-password', requireAdmin, async (req, res) => {
  const { password_lama, password_baru, password_sahkan } = req.body;

  if (!password_lama || !password_baru || !password_sahkan) {
  ...
  if (password_baru !== password_sahkan) {
    return res.status(400).json({ error: 'Kata laluan baru dan pengesahan tidak sepadan.' });
  }

  try {
    const result = await pool.query('SELECT * FROM admins WHERE id = $1', [req.session.admin.id]);
    const admin = result.rows[0];
    if (!bcrypt.compareSync(password_lama, admin.password)) {
      return res.status(401).json({ error: 'Kata laluan lama tidak betul.' });
    }
    const hash = bcrypt.hashSync(password_baru, 10);
    await pool.query('UPDATE admins SET password = $1 WHERE id = $2', [hash, req.session.admin.id]);
    res.json({ success: true });
  } catch(e) {
    res.status(500).json({ error: e.message });
  }
